...
// Effort-without-progress: a script that is fundamentally broken announces
// itself in 2-3 minutes. It swings (or tries to), nothing dies, no rank
// moves, and the same 2-3 commands repeat. The silence/refusal rules below
// need minutes of quiet — this needs only a short window of ACTIVITY THAT
// GOES NOWHERE, so it is evaluated first in the ladder.
const EFFORT_MS = 150000;          // judge from 2m30s (past warm-up)
const EFFORT_WINDOW_MS = 120000;   // look at the last 2 minutes only
const EFFORT_MIN_SWINGS = 6;       // actively trying: >=6 attacks in window
...
export function classifyStall(st, now = Date.now()) {
  ...
  if (elapsed >= EFFORT_MS && Array.isArray(st.swingTimes) && st.rankBaseline) {
    const inWin = st.swingTimes.filter((t) => now - t <= EFFORT_WINDOW_MS).length;
    if (inWin >= EFFORT_MIN_SWINGS) {
      ...
      if (st.kills === 0 && !moved) {
        return {
          verdict: 'wedged',
          reason: `effort with no progress: ${inWin} attacks/${EFFORT_WINDOW_MS / 1000}s, 0 kills, no rank movement [room ${st.room || '?'}]`,
        };
      }
    }
  }
